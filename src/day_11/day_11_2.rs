...
pub(crate) fn run() {
    ...
    let mut row = 0;
    let mut row_empty: Vec<usize> = Vec::new();
    ...
    let mut col = 0;
    let mut col_empty: Vec<usize> = Vec::new();
    ...
    debug!("Row empty: {:?}", row_empty);
    debug!("Col empty: {:?}", col_empty);

    let mut galaxy_distances: Vec<Vec<usize>> = Vec::new();
    // for each pair of galaxies, find the distance between them
    for (i, galaxy) in galaxies.iter().enumerate() {
        for j in i+1..galaxies.len() {
            let distance = find_distance(galaxy, &galaxies[j], &row_empty, &col_empty, 1000000);
...
fn find_distance(galaxy_1: &Vec<usize>, galaxy_2: &Vec<usize>, row_empty: &Vec<usize>, col_empty: &Vec<usize>, empty_multiplier: usize) -> usize {
    let empty_multiplier_minus1 = empty_multiplier - 1;
    ...
    let mut row_adder = 0;
    for row in row_empty.iter() {
        if row < &galaxy_1[1] && row > &galaxy_2[1] {
            row_adder += 1;
        } else if row > &galaxy_1[1] && row < &galaxy_2[1] {
            row_adder += 1;
        }
    }

    let mut col_adder = 0;
    for col in col_empty.iter() {
        if col < &galaxy_1[2] && col > &galaxy_2[2] {
            col_adder += 1;
        } else if col > &galaxy_1[2] && col < &galaxy_2[2] {
            col_adder += 1;
        }
    }
    ...
    debug!("row_adder: {}, col_adder: {}", row_adder, col_adder);


    let distance = ((row_1 - row_2).abs() + (col_1 - col_2).abs() + (empty_multiplier_minus1*col_adder + empty_multiplier_minus1*row_adder) as f64);
