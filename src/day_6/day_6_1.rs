...
pub(crate) fn run() {
    ...
    let time = match_regex(lines.clone(), re_time);
    let distance = match_regex(lines.clone(), re_distance);

    let amount_of_ways_to_win = part_1(time, distance);
...
fn part_1(time: Vec<usize>, distance: Vec<usize>) -> Vec<usize> {
