...
pub(crate) fn run() {
    ...
    for history in histories.iter_mut(){
        debug!("Starting history: {:?}", history.history);
        // go through the history backwards
        for i in (0..history.history.len()-1).rev(){
            let mut predicted = 0;
            predicted += history.history[i].first().unwrap() - history.history[i+1].first().unwrap();

            debug!("Current history: {:?}", history.history[i]);
            debug!("Last element of this history: {:?}", history.history[i].first().unwrap());
            debug!("History below to add: {:?}", history.history[i+1]);
            debug!("Last element of history below: {:?}", history.history[i+1].first().unwrap());
            debug!("Predicted number: {:?}", predicted);


            history.history[i].insert(0,predicted);
